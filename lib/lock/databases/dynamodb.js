...
_.extend(DynamoDB.prototype, {
  ...
  reserve: function(workerId, aggregateId, callback) {
    var params = {
      ...
      Item: { _id: {S: workerId}, aggregateId: {S: aggregateId}, date: {B: (new Date()).toISOString()} },
  ...
  resolve: function(aggregateId, callback) {
    var params = {
      TableName: this.options.collectionName,
      Key: {aggregateId: {S: aggregateId}}
    };
    this.client.deleteItem(params, function (err) {
      debug('resolve', err);
      if (callback) {
        callback(err);
      }
  ...
  getAll: function(aggregateId, callback) {
    ...
    this.client.query(params, function (err, data) {
      ...
      callback(null, _.map(data, function (entry) { return entry._id; }));
    });
  },
